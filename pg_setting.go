...
func querySettings(alias string, ch chan<- prometheus.Metric, db *sql.DB) error {
...
type pgSetting struct {
	alias, name, setting, unit, shortDesc, vartype string
}
...
func (s *pgSetting) metric() prometheus.Metric {
	...
	desc := newDesc(s.alias, subsystem, name)
	return prometheus.MustNewConstMetric(desc, prometheus.GaugeValue, val, shortDesc)
}
